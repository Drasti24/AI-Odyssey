...
function MovieIcon({ icon, selected }) {
  ...
  return (
    <div
      ...
      className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${visual.bg} text-lg font-black text-black shadow-lg transition-transform ${selected ? "scale-110" : ""
        }`}
    >
      {visual.mark}
    </div>
...
export default function NetflixTasteMatcher({ onComplete }) {
  ...
  const selectedGenres = useMemo(
    () => getSelectedGenres(selectedMovies),
    [selectedMovies]
  );

  const selectedMovieDetails = MOVIES.filter((movie) =>
  ...
  const recommendation =
    showRecommendation && selectedMovies.length === 3
      ? neighbors[0]?.liked
      : "Hidden until reveal";
  ...
  const matchStrength =
    selectedGenres.length === 0
      ? 0
      : Math.round(
        (neighbors.reduce((total, viewer) => total + viewer.similarity, 0) /
          (k * maxPossibleMatches)) *
        100
      );
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-[#0f0b0b] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.35)] md:p-6">
      <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            {MOVIES.map((movie) => {
              ...
              return (
                <motion.button
                  ...
                  className={`group overflow-hidden rounded-2xl border bg-black/55 text-left transition-all ${isSelected
                    ...
                    <div className="relative p-4">
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {movie.genres.map((genre) => (
                          <span
                            key={genre}
                            className="rounded-full bg-white/10 px-2 py-0.5 text-[11px] font-bold text-white/65"
                          >
                            {genre}
                          </span>
                        ))}
        ...
        {/* RIGHT SIDE */}
        <section className="flex flex-col gap-5 rounded-2xl border border-white/10 bg-white/[0.035] p-5">
          <div className="flex items-center justify-between rounded-xl border border-cyan-400/20 bg-cyan-400/[0.07] p-4">
            <div>
              ...
              <p className="mt-1 text-xs text-white/55">
                Score updates as you match nearby viewers.
              </p>
            </div>

            <div className="text-3xl font-black text-white">{matchStrength}%</div>
          </div>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedMovieDetails.map((movie) => (
                      <span
                        key={movie.id}
                        className="rounded-lg bg-white/10 px-2.5 py-1 text-xs font-bold text-white/70"
                      >
                        {movie.title}
                      </span>
                    ))}
              ...
              <div className="flex rounded-lg border border-white/10 bg-black/25 p-1">
                {K_OPTIONS.map((option) => (
                  <button
                    key={option}
                    onClick={() => setK(option)}
                    className={`rounded-lg px-4 py-2 text-sm font-black transition-all ${k === option
              ...
              {rankedViewers.slice(0, 3).map((viewer, index) => {
                ...
                return (
                  <motion.div
                    layout
                    key={viewer.id}
                    className={`rounded-xl border p-3 transition-all ${isNeighbor
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-black ${isNeighbor
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {viewer.tastes.map((taste) => (
                        <span
                          key={taste}
                          className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${selectedGenres.includes(taste)
          ...
          <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-4">
              ...
              onClick={
                showRecommendation
                  ? () =>
                    onComplete({
                      points: 40,
                      recommendation: neighbors[0]?.liked,
                      matchStrength,
                      k,
            ...
            >
              {showRecommendation ? "Continue" : "Reveal Result"}
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-white/5 transition-all">
        <button
          onClick={() => setShowHowItWorks(!showHowItWorks)}
          className="flex w-full items-center justify-between p-5 text-left transition-colors hover:bg-white/5"
        >
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-red-600/20 text-red-400">
              <Sparkles size={16} />
            </div>

            <div>
              <span className="block text-lg font-black text-white">
                How the AI made that choice
              </span>
              <span className="text-xs font-medium text-white/50">
                Click to see under the hood of K-Nearest Neighbors
              </span>
            </div>
          </div>
